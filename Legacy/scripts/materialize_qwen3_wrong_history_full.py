...
JOBS = [
    {
        "slug": "gpqa_wrong_history_same_domain",
    ...
    {
        "slug": "gsm8k_wrong_history_cross_domain",
...
def materialize() -> list[Path]:
    ...
    for job in JOBS:
        template = yaml.safe_load(job["template"].read_text(encoding="utf-8"))
        for k in job["ks"]:
            config = deepcopy(template)
            config["experiment"]["name"] = f"{config['experiment']['name']}_{job['slug']}_full_k{k}"
            config["experiment"]["num_samples"] = None
            tags = list(config["experiment"].get("tags", []))
            tags.extend(["wrong_history", "full_samples", job["slug"], f"k{k}"])
            ...
            out = OUTPUT_DIR / f"{job['slug']}__k-{k}.yaml"
